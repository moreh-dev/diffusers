...
class EfficientUNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        if skip_connection_scalings is None:
            skip_connection_scalings = [1.0 for i in range(len(up_block_types))]
        for i, up_block_type in enumerate(up_block_types):
            ...
            up_block = get_up_block(
                ...
                resnet_out_scale_factor=resnet_out_scale_factor,
                cross_attention_norm=cross_attention_norm,
                skip_connection_scaling=skip_connection_scalings[i]
            )
    ...
    def forward(
        ...
        if encoder_attention_mask is not None:
            encoder_attention_mask = (1 - encoder_attention_mask.to(sample.dtype)) * -1e+9
